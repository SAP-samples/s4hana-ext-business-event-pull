sap.ui.define([
	...
	return BaseObject.extend("behsample.model.GroupSortState", {
		 ...
		 * @alias behsample.model.GroupSortState
		 */
		...
		/**
		 * Sorts by SAPObjectTaskTypeName, or by BusEventPriority
		 *
		...
		/**
		 * Groups by BusEventPriority, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "BusEventPriority") {
				// Grouping means sorting so we set the select to the same Entity used for grouping
				this._oViewModel.setProperty("/sortBy", "BusEventPriority");

				aSorters.push(
					new Sorter("BusEventPriority", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "SAPObjectTaskTypeName");
			}
