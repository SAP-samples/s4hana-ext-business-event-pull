				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("BusinessEvent") === oObject.BusinessEvent;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].BusinessEvent;
										} else {
											sCurrentId = aItemsNotInTheList[0].BusinessEvent;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("C_Behqueuedata", {
											BusinessEvent : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("BusinessEvent"),
							title: oBindingContext.getProperty("SAPObjectTaskTypeName")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "<C_Behqueuedata> (0)"}),
							success : function () {
					...
					theListShouldBeSortedAscendingOnUnitNumber : function () {
						return this.theListShouldBeSortedAscendingOnField("BusEventPriority");
					},

					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("SAPObjectTaskTypeName");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all C_Behqueuedata to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "<C_Behqueuedata> (" + iExpectedLength + ")"}),
									success : function () {
